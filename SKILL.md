---
name: wechat-aip-architect
description: This skill should be used when the user wants to build, operate, or optimize a WeChat Official Account "AIP" (AI + personal IP) — including account positioning, monetization setup, viral-content benchmarking, topic selection, article writing, formatting/publishing, or writing-prompt design. Trigger on keywords like 公众号, AIP, 公众号IP, 爆款拆解, 对标, 选题, 排版, 引流, 流量主, 写作提示词, 个人IP.
---

# 公众号 AIP 打造 Skill（WorkBuddy 专用入口）
...
This skill packages a full operational playbook for building a WeChat Official Account around an "AIP" (AI + personal IP) — a vertically-focused account that combines AI-tool expertise with a recognizable personal voice. It covers the complete loop: positioning → account setup → benchmark analysis → topic selection → writing → formatting.

Use this skill to guide users through any phase of the AIP journey, or to produce concrete deliverables (positioning briefs, benchmark templates, article drafts, prompt libraries).
...
2. Load the matching reference(s) for detailed methodology and steps.
